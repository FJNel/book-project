...
router.post("/buckets", requiresAuth, statsLimiter, async (req, res) => {
	...
	const step = Number.parseInt(req.body?.step ?? 1, 10);
	const stepUnit = typeof req.body?.stepUnit === "string" ? req.body.stepUnit.trim().toLowerCase() : "year";
	...
	const startDate = startParse.date;
	const endDateExclusive = endParse.date;
