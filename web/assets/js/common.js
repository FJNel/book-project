...
function setAdminViewContext(payload) {
	const profile = parseUserProfile();
	const userId = Number(payload?.userId);
	if (!profile || profile.role !== 'admin' || !Number.isInteger(userId)) return false;
	if (Number(profile.id) === userId) {
		clearAdminViewContext();
		return false;
	}
	const context = {
		userId,
		displayName: payload?.displayName || payload?.preferredName || payload?.fullName || null,
		email: payload?.email || null,
		setAt: new Date().toISOString()
	};
	try {
		sessionStorage.setItem(ADMIN_VIEW_STORAGE_KEY, JSON.stringify(context));
		return true;
	} catch (err) {
		return false;
	}
...
function clearAdminViewContext() {
	try {
...
function isAdminViewingOtherUser() {
	const profile = parseUserProfile();
	const context = getAdminViewContext();
	if (!profile || !context) return false;
	const targetId = Number(context.userId);
	const currentId = Number(profile.id);
	if (!Number.isInteger(targetId) || !Number.isInteger(currentId)) return false;
	return profile.role === 'admin' && targetId !== currentId;
}

function resolveAdminViewDisplay(context) {
	if (!context) return 'Unknown user';
	return context.displayName || context.email || `User #${context.userId}`;
}

function shouldRenderAdminBanner() {
	const segment = window.location.pathname.split('/').pop().replace(/\.html$/i, '') || '';
	if (!segment) return true;
	if (segment.startsWith('admin')) return false;
	if (segment.startsWith('verify') || segment.startsWith('reset') || segment.startsWith('index')) return false;
	return true;
}
...
function renderAdminViewBanner() {
	...
	const context = getAdminViewContext();
	const isViewingOther = isAdminViewingOtherUser();
	const existing = document.getElementById('adminViewBanner');
	if (existing) existing.remove();
	if (!context || !isViewingOther || !shouldRenderAdminBanner()) return;
	const container = document.getElementById('navbarContainer') || document.body;
	const banner = document.createElement('div');
	banner.id = 'adminViewBanner';
	banner.className = 'alert alert-secondary border-0 rounded-0 mb-0';
	banner.innerHTML = `
		<div class="container d-flex flex-wrap align-items-center justify-content-between gap-2">
			<div>
				<div class="fw-semibold">Viewing library: <span id="adminViewDisplay"></span></div>
				<div class="small text-muted">Read-only mode for this library.</div>
			</div>
			<div class="d-flex align-items-center gap-2">
				<a class="btn btn-sm btn-outline-secondary" href="admin#usersSection">Admin users</a>
				<button type="button" class="btn btn-sm btn-outline-primary" id="exitAdminViewBtn">Exit view</button>
			</div>
		</div>
	`;
	...
	const exitBtn = banner.querySelector('#exitAdminViewBtn');
	const display = banner.querySelector('#adminViewDisplay');
	if (display) {
		display.textContent = resolveAdminViewDisplay(context);
	}
	if (exitBtn) {
...
function ensureReadOnlyModal() {
	if (document.getElementById('readOnlyModeModal')) return;
	const modal = document.createElement('div');
	modal.className = 'modal fade';
	modal.id = 'readOnlyModeModal';
	modal.tabIndex = -1;
	...
	modal.innerHTML = `
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Read-only mode</h5>
					<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
				</div>
				<div class="modal-body">
					<p class="mb-0" id="readOnlyModeModalBody">You are viewing another library. Changes are disabled in this mode.</p>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Close</button>
					<button type="button" class="btn btn-primary" id="readOnlyExitBtn">Exit view</button>
				</div>
			</div>
		</div>
	`;
	document.body.appendChild(modal);
	const exitBtn = modal.querySelector('#readOnlyExitBtn');
	if (exitBtn) {
		exitBtn.addEventListener('click', () => {
			clearAdminViewContext();
			window.location.href = 'books';
...
function showReadOnlyModeModal(context) {
	ensureReadOnlyModal();
	const modal = document.getElementById('readOnlyModeModal');
	const body = document.getElementById('readOnlyModeModalBody');
	if (body) {
		body.textContent = `You’re viewing ${resolveAdminViewDisplay(context)}’s library. Changes are disabled in this mode.`;
	}
	if (window.modalManager && typeof window.modalManager.showModal === 'function') {
		window.modalManager.showModal(modal, { backdrop: 'static', keyboard: false });
		return;
	}
	if (window.bootstrap && window.bootstrap.Modal) {
...
window.adminViewContext = {
	getContext: getAdminViewContext,
	setContext: setAdminViewContext,
	clearContext: clearAdminViewContext,
	isViewingOtherUser: isAdminViewingOtherUser
};
window.showReadOnlyModeModal = showReadOnlyModeModal;
