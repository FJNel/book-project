...
Devices and Sessions refer to each unique device or browser instance that you use to access your Book Project account. This includes computers, smartphones, tablets, and web browsers.

When you log in to your Book Project account from a new device or browser, a new session is created. Each session is associated with a specific device or browser and has its own unique identifier to keep you logged in.

## Why should I manage my devices and sessions?

Managing your devices and sessions is important for several reasons:
- **Security**: Regularly reviewing your active sessions helps you identify any unauthorized access to your account. If you notice a session from an unfamiliar device or location, you can log it out immediately to protect your account.
- **Convenience**: Keeping track of your active sessions allows you to manage where you are logged in. You can log out from devices you no longer use or from public/shared devices to ensure your account remains secure.
- **Account Control**: By managing your sessions, you can ensure that only trusted devices have access to your account, reducing the risk of unauthorized access.

## How to Manage Your Devices and Sessions
...
### What is considered a device or session?
A device or session refers to each unique device or browser instance that you use to access your Book Project account. This includes computers, smartphones, tablets, and web browsers. Each time you log in from a new device or browser, a new session is created. This means a session is tied to a specific device or browser and has its own unique identifier to keep you logged in. 

### Why are there multiple sessions listed for the same device or browser?
Multiple sessions may be listed for the same device or browser if you have logged in multiple times from that device or browser. Each time you log in, a new session is created, even if it's from the same device or browser. This can happen if you clear your browser cookies and log in again. Each session will have its own unique identifier and expiration time. 

### Does opening a new tab or window create a new session?
...
### What happens when I log out manually from a device or browser?
When you log out manually from a device or browser, the session associated with that device or browser is terminated immediately. You will be logged out of your Book Project account on that specific device or browser, and you will need to log in again to create a new session if you wish to access your account from that device or browser in the future. Manual logout helps ensure that your account remains secure, especially when using public or shared devices.

### Can someone stay logged in after I change my password?
No, when you change your password, all active sessions associated with your account are invalidated. This means that anyone who is currently logged in on any device or browser will be logged out and will need to log in again using the new password. This security measure helps protect your account by ensuring that only users with the updated credentials can maintain access after a password change.

### Can someone stay logged in after I change my email address?
No, when you change your email address, all active sessions associated with your account are invalidated. This means that anyone who is currently logged in on any device or browser will be logged out and will need to log in again using the updated email address. This security measure helps protect your account by ensuring that only users with the correct credentials can maintain access after an email address change.

### Is it safe to leave sessions active on my own devices?
Yes, it is generally safe to leave sessions active on your own trusted devices, such as your personal computer, smartphone, or tablet. However, it is important to ensure that these devices are secure and not accessible to unauthorized users.

### Are sessions encrypted and protected?
Yes, sessions in The Book Project are encrypted and protected using standard security practices. We use a two-layer approach to secure sessions:
1. **15-minute short-lived access tokens**: These tokens are used for contacting our services and expire after 15 minutes. This limits the time window in which a token can be used if it were to be intercepted by a malicious actor.
2. **7-day refresh tokens**: These tokens are used to obtain new access tokens and expire after 7 days. This means that even if an attacker were to obtain a refresh token, they would only have a limited time to use it before it becomes invalid.

### What's the difference between logging out of a single session and logging out of all sessions?
Logging out of a single session terminates the session associated with a specific device or browser, while logging out of all sessions terminates all active sessions across all devices and browsers. This means that when you log out of a single session, you will only be logged out from that specific device or browser, whereas logging out of all sessions will log you out from every device and browser where you are currently logged in.

### Will logging out of another session affect my current session?
No, logging out of another session will not affect your current session. When you log out of a specific session, only that session is terminated, and you will remain logged in on your current device or browser. This allows you to manage your sessions without disrupting your current access to your Book Project account.
...
You are automatically logged out of all sessions when you change your password or your email address. Changing your password is another good way to ensure that all sessions are logged out and that only users with the updated credentials can maintain access to your account.

### I accidentally logged out of all sessions. Can I revert this?
...
### A session disappeared from the list without me logging out. Why?
Sessions can disappear from the list for a few reasons:
- **Expiration**: Sessions are set to expire 7 days after they are issued. If a session reaches its expiration time, it will be automatically removed from the list.
- **Manual Logout**: If you or someone with access to your account manually logged out of a session, it will be removed from the list.
- **Account Changes**: If you changed your password or email address, all active sessions would be invalidated and removed from the list.
- **System Maintenance**: Occasionally, system maintenance or updates may result in sessions being cleared or removed.
